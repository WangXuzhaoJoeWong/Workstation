...
- 如果你确实要改端口，建议用 systemd 的 per-unit `Environment=` 单独覆盖。
...
如果你改了 `WXZ_METRICS_HTTP_PATH`，同步修改 `metrics_path`。
...
- 如果你希望 bt_service 收到 fatal fault 就退出并由 systemd 拉起：
...
- 如果你希望 arm_control 收到 fatal/error fault 就写 marker（不退出）：
...
注意：如果两个服务共用同一份 `EnvironmentFile=`，但你希望它们采用不同的 fault recovery 策略，建议同样用 per-unit 的 `Environment=` 做覆盖。
